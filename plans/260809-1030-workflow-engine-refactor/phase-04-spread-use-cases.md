...
B3: Chuyển 5 use case còn lại qua CaseTransitionService. BE: filter allowed_transitions theo stage. FE: render nút theo danh sách (field type đã có sẵn). **Vô hiệu route cũ khi use case chuyển xong** (F5: chống split-brain 2 engine). **Xóa bảng tay `isValidStageTransition`** (F11: hết 3 nguồn truth).
...
4. Sửa `get-case-detail.usecase.ts`: filter `allowed_transitions` theo stage hiện tại
5. GIỮ NGUYÊN: veto (T13), resubmit (T3/T4), completeCase (T14) — logic cũ
...
- Transition CHƯA chuyển (T6/T7/T8/T10 — tùy quyết định phase này) vẫn qua symflow path → flag per-transition
...
| `apps/api/src/modules/cases/application/update-case-status.usecase.ts` | **SỬA** | F5: vô hiệu transition đã chuyển (feature flag per-transition) |
...
- [ ] **F5:** sửa `update-case-status.usecase.ts` — feature flag per-transition, vô hiệu transition đã chuyển. Grep `applyTransition` → chỉ transition chưa chuyển còn gọi
