...
| `CENTRIFUGO_TOKEN_SECRET` | API ký JWT **và** container `CENTRIFUGO_CLIENT_TOKEN_HMAC_SECRET_KEY` | Phải **giống hệt** 2 nơi, không → WS connect/subscribe 401 |
| `CENTRIFUGO_API_KEY` | API HTTP publish **và** container `CENTRIFUGO_HTTP_API_KEY` | Phải khớp, không → publish non-200 |
...
| `CENTRIFUGO_EXPOSED_PORT` | container `ports` host bind | Local: mặc định `8010` · Prod: để trống bỏ bind (`127.0.0.1:${CENTRIFUGO_EXPOSED_PORT:-8010}:8000`) |
...
Lý do: browser dev gửi `Origin: http://localhost:3001`, allow-list `https://...` → Centrifugo chặn WS.

> ⚠️ `centrifugo/config.json` hardcode `allowed_origins: ["http://localhost:3001"]` + secret `changeme`. Env container override config khi set. Set đủ env, không dựa file config.

## Port mapping

Centrifugo config server port `8000` (trong container). Host port do `CENTRIFUGO_EXPOSED_PORT`:

- **Local**: bind `127.0.0.1:8010` → container `8000`. API/web host truy cập qua `localhost:8010`.
...
Kiểm theo thứ tự (từ incident 2026-08-08):

1. **Container expose host port?** `docker inspect nexus-centrifugo --format '{{json .HostConfig.PortBindings}}'` → `{}` = API/web host không tới được → chỉ polling 60s. Fix: set `CENTRIFUGO_EXPOSED_PORT` + `up -d --force-recreate centrifugo`.
2. **`allowed_origins` chứa origin trình duyệt?** Kiểm: `docker inspect nexus-centrifugo` → `CENTRIFUGO_CLIENT_ALLOWED_ORIGINS`. Dev phải có `http://localhost:3001`.
3. **API publish fail?** Log API dev (`apps/api/logs/combined.log`): tìm `centrifugo publish failed` / `ECONNREFUSED`. Sai host/port/`CENTRIFUGO_URL` hoặc thiếu `CENTRIFUGO_API_KEY`.
4. **Token secret khớp?** `CENTRIFUGO_TOKEN_SECRET` (API) === container `CENTRIFUGO_CLIENT_TOKEN_HMAC_SECRET_KEY`. Lệch → WS 401, sub không bao giờ publication.
...
- Publish fire-and-forget (`void publishToChannel(...).catch(log)`) — Centrifugo down không fail request chính, tin vẫn lưu DB, polling fallback tự bù.
