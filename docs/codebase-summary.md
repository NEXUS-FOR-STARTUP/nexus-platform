...
- Settings & UserMenu modal (2026-08-13, spec F07 `docs/requirements/settings-sidebar-and-profile.md`): khu vực `/dashboard/settings` — nested layout sidebar (pattern Facebook): `/dashboard/settings` → redirect profile, `/dashboard/settings/profile` (Thông tin cơ bản: tên hiển thị + email read-only full + avatar "Đổi ảnh" thông báo đang phát triển), `/dashboard/settings/password` (Đổi mật khẩu, `revokeOtherSessions: true`). Route cũ `/dashboard/profile` → redirect stub. Avatar trong `DashboardShell` mở Mantine `Modal` (`components/layout/_components/UserMenu.tsx`, thay dropdown cũ): student — Hồ sơ/Thanh toán/Cài đặt + số dư ví + Đăng xuất; admin/supporter — workspace link + Đăng xuất. Hook `useProfileMutations` (`app/dashboard/settings/hooks/`); `lib/auth-errors.ts` `translateAuthError` dùng chung. `DashboardShell` 200 → 60 dòng.
